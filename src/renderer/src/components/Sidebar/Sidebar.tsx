...
const Sidebar: React.FC<SidebarProps> = ({ className, activeItem = 'dashboard', onItemClick }) => {
  ...
  const handleItemClick = (itemId: string, hasSubmenu: boolean) => {
    // Sempre chama onItemClick se foi fornecido
    if (onItemClick) {
      onItemClick(itemId)
    }

    // Se tem submenu, toggle o expandido
    if (hasSubmenu) {
  ...
  return (
    <div className={`w-64 h-screen neutral-bg flex flex-col ${className || ''}`}>
      {/* Header */}
      ...
      <div className="flex-1 !font-bold">
        {menuItems.map((item) => {
          ...
          return (
            <div key={item.id} className="px-4 !mb-1.5">
              <button
                onClick={() => handleItemClick(item.id, item.hasSubmenu)}
                className={`
                  w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-xs font-bold transition-all duration-200 text-left group
                  ${
                    isActive
              ...
              {item.hasSubmenu && isExpanded && item.submenu && (
                <div className="mt-1 ml-8 !space-y-1">
                  {item.submenu.map((subItem, index) => (
                    <button
                      key={index}
                      className="w-full text-left !mt-2 px-3 py-1.5 text-xs text-gray-500 hover:text-gray-700 rounded-md hover:bg-gray-50 transition-colors flex items-center font-bold gap-2"
                    >
